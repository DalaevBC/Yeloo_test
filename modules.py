...
class FunctionModule:
    ...
    def startReferral(self, message):
        ...
        # ѕровер€ем наличие хоть какой-то дополнительной информации из ссылки
        if " " in message.text:
            invited_id = message.text.split(' ')[1]
            # ѕробуем преобразовать строку в число
            try:
                invited_id = int(invited_id)
                # ѕровер€ем на несоответствие TG ID пользовател€ TG ID реферера
                if user_id != invited_id:
                    # ѕровер€ем, есть ли такой реферер в базе данных
    ...
    def getMyReferrals(self, followed_referrals):
        # возвращаем список пользователей, которые прошли по нашей ссылке
